...
namespace TravelUpdate.Controllers
{
    ...
    public class HotelImageController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult<InputImage>> CreateHotelImage([FromForm] InputImage inputImage, [FromQuery] string? customUrl = null)
        {
            ...
            var url = customUrl ?? "gethotelimage";
            //return CreatedAtAction(nameof(GetHotelImage), new { id = hotelImage.HotelID }, new { HotelImageID = hotelImage.HotelID, Url = url });
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateHotelImage(int id, [FromForm] InputImage inputImage, [FromQuery] string? customUrl = null)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            var url = customUrl ?? "gethotelimage";
            return Ok(new { HotelImageID = hotelImage.HotelImageID, Url = url });
        }
