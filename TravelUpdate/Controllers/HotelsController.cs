...
namespace TravelUpdate.Controllers
{
    ...
    public class HotelsController : ControllerBase
    {
        ...
        // POST api/Hotels
        [HttpPost]
        public async Task<ActionResult<int>> CreateHotel([FromBody] FacilityWiseHotel facilityWiseHotel, [FromQuery]string?customUrl=null)
        {
            ...
            var url = customUrl ?? "gethotel";

            // return CreatedAtAction(nameof(GetHotel), new { id = hotel.HotelID }, hotel.HotelID);
            return Ok(new { HotelID = hotel.HotelID, Url = url });
        }

        // PUT api/Hotels/5
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateHotel(int id, [FromBody] FacilityWiseHotel facilityWiseHotel, [FromQuery] string? customUrl = null)
        {
